...
def load_images(path, split):
    ...
    for sub_path in glob.glob(os.path.join(path, '*' + postfix)):
        for ext in image_extensions:
            # print("Looking for images in", os.path.join(sub_path, "*.{}".format(ext)))
...
# python fid.py --path /home/peixixio/LatteGAN/results/experiments/exp182-geneva-iclevr/images_test --batch-size 32
if __name__ == "__main__":
    ...
    assert options.path is not None, "--path is an required option"
    ...
    print(options.path, 'FID is:', fid_value)