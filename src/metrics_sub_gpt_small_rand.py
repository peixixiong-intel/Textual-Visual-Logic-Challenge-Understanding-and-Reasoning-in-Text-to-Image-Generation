...
class ImageFolderNonGT(Dataset):
        ...
        with h5py.File(dataset_path, "r") as f:
            ...
            if category == 'general_all':
                ...
                for k in _keys:
                    ...
                    gen_name = '*_' + scene_id + '.png'  # 0000
                    old_gen_image = os.path.join(new_img_folders, gen_name)
                ...
                for k in f.keys():
                    if k.isdigit():
                        ...
                        if category == cate:
                            ...
                            # print([int(numb) for numb in gen_dirs])
                            # assert int(scene_id) in [int(numb) for numb in gen_dirs]
        print('self.id2idx', len(self.id2idx))
    ...
    def __getitem__(self, idx):
        ...
        new_size = gt_sample.shape
        # Resize the image
        gen_sample = cv2.resize(gen_sample, (new_size[1], new_size[0]))

        # load from h5 dataset
        scene_id = gen_path.split("/")[-2]
        i = self.id2idx[scene_id]
        j = -1 #int(gen_path.split("/")[-1].split(".")[0])
...
def relink_img(category, ori_img_folder, new_img_folder, ref_img_folder, ori_idx):
    if 'general_all' in ref_img_folder or 'general_all_rsim' in ref_img_folder:
        return ori_idx
    try:
    ...
    old_gen_images = glob.glob(ori_img_folder + '/*_4.png')

    for ref_sub_folder in tqdm(glob.glob(ref_img_folder + '/*_gt')):
        sub_name = os.path.basename(ref_sub_folder) #'0000_gt'
        old_gen_image = old_gen_images[ori_idx]
        # print('old_gen_image', old_gen_image)
        dst_folder = os.path.join(new_img_folder, sub_name)
        dst_folder_all1 = os.path.join(new_img_folder.replace(category, 'general_all'), sub_name)
        dst_folder_all2 = os.path.join(new_img_folder.replace(category, 'general_all_rsim'), sub_name)
        # print('dst_folder', dst_folder)
        if os.path.exists(os.path.join(ref_sub_folder, '0.png')):
            os.makedirs(dst_folder)
            os.makedirs(dst_folder_all1)
            os.makedirs(dst_folder_all2)
            os.symlink(os.path.join(ref_sub_folder, '0.png'), os.path.join(dst_folder, '0.png'))
            os.symlink(os.path.join(ref_sub_folder, '0.png'), os.path.join(dst_folder_all1, '0.png'))
            if category != 'numb':
                os.symlink(os.path.join(ref_sub_folder, '0.png'), os.path.join(dst_folder_all2, '0.png'))
            ...
            new_sub_folder_all1 = os.path.join(new_img_folder.replace(category, 'general_all'), sub_name.replace('_gt', ''))
            os.makedirs(new_sub_folder_all1)

            os.symlink(old_gen_image, os.path.join(new_sub_folder, '0.png'))
            os.symlink(old_gen_image, os.path.join(new_sub_folder_all1, '0.png'))

            if category != 'numb':
                new_sub_folder_all2 = os.path.join(new_img_folder.replace(category, 'general_all_rsim'), sub_name.replace('_gt', ''))
                os.makedirs(new_sub_folder_all2)
                os.symlink(old_gen_image, os.path.join(new_sub_folder_all2, '0.png'))


            os.symlink(old_gen_image, os.path.join(new_img_folder.replace(category, ''), 'rand_' + sub_name.replace('_gt', '')+'.png'))
            # print('case 2', old_gen_image, os.path.join(new_sub_folder, '0.png'))
            ori_idx += 1
            # if 'general_all' in ref_img_folder or 'general_all_rsim' in ref_img_folder:
                # if ori_idx >= 50:
                #     break
            if 'general_all' not in ref_img_folder or 'general_all_rsim' not in ref_img_folder:
                if ori_idx >= 10:
                    break
    return ori_idx
...
if __name__ == '__main__':
    # ori_img_folder = '/net/csr-dgx1-04/data2/peixixio/GeNeVA_datasets/GPT_images' #'/home/peixixio/U-ViT/assets/results' # '/home/peixixio/dalle/out_coco' #'/home/peixixio/LatteGAN/results/out_clevr_new_dalle' #'/home/peixixio/LatteGAN/results/out_codraw_dalle' #'/home/peixixio/LatteGAN/results/out_img_dalle' #'/home/peixixio/LatteGAN/results/uvit_codraw' #'/home/peixixio/LatteGAN/results/out_images_uvit'
    # new_img_folders = '/home/peixixio/LatteGAN/results/experiments/GPT_images' #'/home/peixixio/LatteGAN/results/experiments/uvit_lrt2i' # '/home/peixixio/LatteGAN/results/experiments/dalle_lrt2i' # '/home/peixixio/LatteGAN/results/experiments/dalle_iclevr_new' #'/home/peixixio/LatteGAN/results/experiments/dalle_codraw' #'/home/peixixio/LatteGAN/results/experiments/dalle_iclevr' #'/home/peixixio/LatteGAN/results/experiments/uvit_codraw' #'/home/peixixio/LatteGAN/results/experiments/uvit'
    # ori_img_folder = '/net/csr-dgx1-04/data2/peixixio/exp-results/experiments/GPT_images_small_inter_2'
    # new_img_folders = '/net/csr-dgx1-04/data2/peixixio/exp-results/experiments/GPT_images_small_inter_2'
    ori_img_folder = '/net/csr-dgx1-04/data2/peixixio/exp-results/experiments/img_rand'
    new_img_folders = '/net/csr-dgx1-04/data2/peixixio/exp-results/experiments/GPT_images_small_rand'
    os.makedirs(new_img_folders, exist_ok=True)
    ref_img_folders = '/net/csr-dgx1-04/data2/peixixio/exp-results/experiments/exp183-geneva-LRT2I-new-eval/images_test/' #'/home/peixixio/LatteGAN/results/experiments/exp169-geneva-codraw/images_test' #'/home/peixixio/LatteGAN/results/experiments/exp170-geneva-iclevr/images_test'
    try:
        shutil.rmtree(new_img_folders)
        shutil.rmtree(os.path.join(new_img_folders, 'general_all'))
        shutil.rmtree(os.path.join(new_img_folders, 'general_all_rsim'))
    except:
        pass
    for ref_img_folder in glob.glob(os.path.join(ref_img_folders, '*')):
        ...
        if sub_cate == 'general_all' or sub_cate == 'general_all_rsim':
            continue
        new_img_folder = os.path.join(new_img_folders, sub_cate)
        print('new_img_folder', new_img_folder)
        ori_idx = 0
        ori_idx = relink_img(sub_cate, ori_img_folder, new_img_folder, ref_img_folder, ori_idx)
        ...
        with open(sub_cate + '_gpt4_rand_small_result.json', 'w', encoding='utf-8') as f:
