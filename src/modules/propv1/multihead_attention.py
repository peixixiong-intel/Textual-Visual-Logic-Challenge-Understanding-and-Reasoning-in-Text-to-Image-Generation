...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        if d_model % nhead != 0:
            raise ValueError
        # print('query_features', query_features, 'value_features', value_features, 'key_features', key_features)
        ...
        if use_spectral_norm:
            ...
            nn.init.orthogonal_(self.fc_r.weight.data)
        else:
    ...
    def forward(self, q, k, v, key_padding_mask=None, rel_mask=None):
        """multihead attention.
        ...
        Args:
            ...
            torch.Size([16, 256, 512]) torch.Size([16, 91, 768])
            key_padding_mask (torch.tensor, optional): shape=(B, L) for padding. Defaults to None.
        ...
        if self.rel_enhancement and rel_mask != None:
            rel_mask = rel_mask.repeat(1, 1, v.size(-1))
            if rel_mask.size() != v.size():
                r = self.fc_r(rel_mask)
            else:
                r = rel_mask
            v = v*r
